...
void GraphicsArenaViewer::OnLeftMouseDown(int x, int y) {
  std::cout << "Left mouse button DOWN (" << x << ", "
  << y << ")" << std::endl;
}
...
void GraphicsArenaViewer::OnRightMouseUp(int x, int y) {
  std::cout << "Right mouse button UP (" << x << ", "
  << y << ")" << std::endl;
}
...
void GraphicsArenaViewer::DrawRobotSensors(NVGcontext *ctx, double xpos,
...
void GraphicsArenaViewer::DrawUsingNanoVG(NVGcontext *ctx) {
  ...
  for (size_t i = 0; i < robots.size(); i++) {
    DrawRobotSensors(ctx, robots[i]->get_pos().x, robots[i]->get_pos().y,
    robots[i]->get_speed()*cos(robots[i]->get_heading_angle()*PI/180),
    robots[i]->get_speed()*sin(robots[i]->get_heading_angle()*PI/180),
    robots[i]->get_radius()*PI/180*5,
    robots[i]->get_fov());
    }
