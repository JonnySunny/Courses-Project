 ...
 */
  class SensorProximity : public Sensor {
    ...
    */
    ...
    /**
    * @brief Return a angle with in 0-360 to avoid the unexpected situations when computing the distance
    * It's a  helper functin for sensor_reading.
    * @return angle int
    */
    int Positive(int angle) {
      if (angle < 0) {
        return angle + 360.0;
      }
      if (angle > 360) {
    ...
    // SensorProximity& operator=(const SensorProximity& other);
    // SensorProximity(const SensorProximity& other);
