...
public class BouncingBall extends AnimationFrame{
	...
	public void action(){
		...
		if ( ballX<BORDER || ballX+ballSize>getWidth()-BORDER ){
			ballXSpeed*=-1;
		}
		if ( ballY<BORDER || ballY+ballSize>getHeight()-BORDER ){
			ballYSpeed*=-1;
		}
     ...
     protected void processKeyEvent(KeyEvent e) {
        ...
        */
        if (e.getID() == KeyEvent.KEY_PRESSED && keyCode == KeyEvent.VK_P) {
            ...
            try {
                image.save("heatmap1"+saveCounter+ ".png", EasyBufferedImage.PNG);
                saveCounter++;
